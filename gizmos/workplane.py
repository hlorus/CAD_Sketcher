...
# Margin added around a hovered face's bounding box, as a fraction of its larger
# side, when previewing the workplane it would create.
FACE_PREVIEW_MARGIN = 0.15


def _plane_color(pick_id, is_hovered, ts):
...
def _draw_rect(context, mat, bounds, scale, col, is_hovered):
...
def resolve_picker_hover(context, coords) -> tuple:
    """What a workplane pick at ``coords`` would use, for hover feedback.

    Mirrors the pick priority (border > mesh > interior) so the feedback always
    matches what a click would do. Returns ``(hover_pick_id, mesh_preview)``
    where ``mesh_preview`` is ``(object_name, face_index)`` or None.
    """
    kind, a, b = resolve_sketch_base(context, Vector(coords))
    if kind in ("border", "interior"):
        return a, None  # a = workplane pick id
    if kind == "mesh":
        return "", (a.name, b)  # a = object, b = face index
    return "", None


def draw_workplane_picker(context, hover, mesh_preview) -> None:
    """Draw every pickable workplane plus the plane a hovered face would create.

    Shared by the Add Sketch gizmo and the Change Workplane operator so both
    pickers look the same.
    """
    scale = preferences.get_scale()
    ts = get_prefs().theme_settings.entity

    for wp_obj, pick_id in iter_wp_empties(context):
        is_hovered = hover == pick_id
        col = _plane_color(pick_id, is_hovered, ts)
        bounds = wp_plane_bounds(context, pick_id)
        _draw_rect(context, wp_obj.matrix_world, bounds, scale, col, is_hovered)

    # Preview the workplane a hovered mesh face would create, sized to the
    # face's bounding box plus a margin.
    if not mesh_preview:
        return
    ob = bpy.data.objects.get(mesh_preview[0])
    if ob is None:
        return
    try:
        mat = face_workplane_matrix(context, ob, mesh_preview[1])
        bounds = face_bounds_in_plane(context, ob, mesh_preview[1], mat)
    except (IndexError, AttributeError):
        return
    if bounds is None:
        return
    min_x, min_y, max_x, max_y = bounds
    margin = max(max_x - min_x, max_y - min_y) * FACE_PREVIEW_MARGIN
    bounds = (min_x - margin, min_y - margin, max_x + margin, max_y + margin)
    _draw_rect(context, mat, bounds, scale, ts.highlight, True)
...
class VIEW3D_GT_slvs_workplane(Gizmo):
    ...
    def draw(self, context):
        draw_workplane_picker(
            context, selection.hover, getattr(self, "mesh_preview", None)
        )

    def test_select(self, context, location):
        new_hover, preview = resolve_picker_hover(context, location)
