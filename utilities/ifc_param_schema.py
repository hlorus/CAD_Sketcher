...
def get_display_schema(tag_value: str) -> list[dict]:
    return list(DISPLAY_SCHEMA.get((tag_value or "").strip(), ()))
...
def _read_display_value(source: str, element_type):
    if source == "entity_name":
        return getattr(element_type, "Name", None) or element_type.is_a()

    layer_set = _get_material_layer_set(element_type)
...
def get_ifc_type_display_rows(tag_value: str, type_guid: str) -> list[tuple[str, str]]:
    schema = get_display_schema(tag_value)
    if not schema:
        return []

    type_guid = (type_guid or "").strip()
    if not type_guid:
        return []
    ...
    element_type = _resolve_ifc_element(ifc_file, type_guid)
    if element_type is None:
        return []

    rows = []
    for item in schema:
        value = _read_display_value(item.get("source", ""), element_type)
