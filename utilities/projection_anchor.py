...
def bind_projected_point(sketch, point, source, vertex_index):
    ...
    attributes[PROJECT_LAST_CO_ATTR].data[curve_index].vector = source.data.vertices[vertex_index].co
    return vertex_id
...
def refresh_projection_for_sketch(sketch, depsgraph, changed=None, force=False):
    ...
    updates = {}
    source_cache = {}
    for curve_id, source, vertex_id, fallback_index, last_co in list(iter_projected_point_bindings(sketch)):
...
def project_mesh_object(sketch, source, construction=True):
    ...
    with batch_update(sketch):
        for vertex_index in used_indices:
            ...
            point = PointRef.create(sketch, (local.x, local.y), construction=construction, fixed=True, name="Projected Point")
        ...
        for edge in mesh.edges:
            ...
            if p1 is None or p2 is None:
                continue
            line = LineRef.create(sketch, p1, p2, construction=construction, name="Projected Line")
            lines.append(line)
    return points, lines
